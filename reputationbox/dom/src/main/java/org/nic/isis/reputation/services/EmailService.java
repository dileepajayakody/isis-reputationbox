...
public class EmailService {
	...
	public List<EmailViewModel> allMessages() {
		...
		params.put("since", "0");
		params.put("limit", "20");
		ContextIOResponse cio = contextio.allMessages("gdc2013demo@gmail.com",
				params);
		...
		for (int i = 0; i < data.length(); i++) {
			...
			// EmailViewModel evm =
			// container.newViewModelInstance(EmailViewModel.class,jsonObj.toString());
			EmailViewModel evm = new EmailViewModel();
			try {
				evm.viewModelInit(URLEncoder.encode(jsonObj.toString(), "UTF-8"));
